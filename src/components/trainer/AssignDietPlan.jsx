...
const AssignDietPlan = ({ dietPlans, userId, currUserDietPlan }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        start_date: userDietPlan?.start_date?.slice(0, 10) || "",
    ...
    const handleAssignDietPlan = async (e) => {
        ...
        try {
            setError("")
            const result = await createUserDietPlan(formData, auth.token);
            if (result.success) {
                setUserDietPlan(result.data)
                setEditMode(false)
            } else {
                console.log(result);
                setError(result);
        ...
        <div>
            <div className="bg-white rounded-2xl shadow p-5 ">
                {(userDietPlan && !editMode) ? (
                        ...
                        <div>
                            <p className="text-sm mb-1">
                                ...
                                <button
                                    ...
                                    onClick={() => navigate(`/trainer/adddietplan/${userDietPlan.diet_plan_id?._id}`)}
                                >
                                    {userDietPlan.diet_plan_id?.plan_name}
                                </button>
                            </p>
                            <p className="text-xs text-gray-600">
                                {new Date(userDietPlan.start_date).toLocaleDateString()} →{" "}
                    ...
                    <form className="grid grid-cols-3 gap-3 text-sm" onSubmit={handleAssignDietPlan} >
                        <div className="col-span-3">
                            <label className="block mb-1 font-medium">Diet Plan</label>
                            <select className="w-full border rounded px-2 py-1" value={formData.diet_plan_id} name="diet_plan_id"
