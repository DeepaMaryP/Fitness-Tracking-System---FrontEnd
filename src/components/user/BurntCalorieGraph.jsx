...
function BurntCalorieGraph({userId}) {  
    ...
    useEffect(() => {
        filterGraphWorkOutTracks(7); // 7 days before by default - graph
    }, [])

    // Filter graph based on selection
    const filterGraphWorkOutTracks = (range) => {
        setGraphPeriod(range)
        const endDate = new Date();
        const startDate = new Date(endDate);
        startDate.setDate(endDate.getDate() - range);
        fetchWorkOutTrackerLogs(userId, startDate, endDate, 'graph');
    };

    const fetchWorkOutTrackerLogs = async (userId, startDate, endDate, type = "card") => {
        try {
            ...
            if (result.success && result.data) {
                setGraphWorkOutHistory(result.data)
            }
        } catch (err) {
            console.warn("No WorkOut tracker found");
            setGraphWorkOutHistory([])
        }
    };

    // Prepare graph data
    const prepareGraphData = () => {
        let data = [...graphWorkOutHistory].sort((a, b) => new Date(a.date) - new Date(b.date));
...
export default BurntCalorieGraph
